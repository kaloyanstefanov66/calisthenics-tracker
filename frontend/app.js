...
const validTabs = ["calisthenics-page", "gym-page", "history-page", "ai-page"];
...
if (chatForm) {
    chatForm.addEventListener("submit", async (e) => {
        ...
        userMsgDiv.style.cssText = "max-width: 85%; padding: 12px 16px; border-radius: 8px; border-right: 2px solid var(--accent); background: rgba(158, 184, 108, 0.08); line-height: 1.6; font-size: 13px; margin-left: auto; text-align: left; color: #fff;";
        userMsgDiv.innerHTML = `<strong>YOU:</strong> ${escapeHtml(message)}`;
        chatMessages.appendChild(userMsgDiv);
        chatMessages.scrollTop = chatMessages.scrollHeight;

        // Append "APEX Coach is typing..." loading indicator
        const typingIndicator = document.createElement("div");
        typingIndicator.className = "chat-message bot typing";
        typingIndicator.style.cssText = "max-width: 85%; padding: 12px 16px; border-radius: 8px; border-left: 2px solid var(--accent); background: rgba(255,255,255,0.03); line-height: 1.6; font-size: 13px; color: #888; font-style: italic;";
        typingIndicator.innerHTML = `<strong>APEX AI:</strong> Coach is compiling your response...`;
        ...
        try {
            ...
            if (!response.ok) {
                throw new Error("Failed to get response from APEX Coach");
            }

            const data = await response.json();
            const coachReply = data.text || "I apologize, but I could not formulate a response. Please try again.";

            // Save to Local history memory
            chatHistory.push({ role: "user", text: message });
            chatHistory.push({ role: "model", text: coachReply });
            // Limit history to last 15 messages for context size safety
            if (chatHistory.length > 30) chatHistory = chatHistory.slice(-30);

            // Append Coach Reply
            const botMsgDiv = document.createElement("div");
            botMsgDiv.className = "chat-message bot";
            botMsgDiv.style.cssText = "max-width: 85%; padding: 12px 16px; border-radius: 8px; border-left: 2px solid var(--accent); background: rgba(255,255,255,0.03); line-height: 1.6; font-size: 13px; color: #d4d4d4;";
            
            // Render markdown lists and bold formatting elegantly
            botMsgDiv.innerHTML = `<strong>APEX AI:</strong> ${renderBotReply(coachReply)}`;
            ...
            errorMsgDiv.style.cssText = "max-width: 85%; padding: 12px 16px; border-radius: 8px; border-left: 2px solid #ff4444; background: rgba(255,68,68,0.03); line-height: 1.6; font-size: 13px; color: #ff6b6b;";
            errorMsgDiv.innerHTML = `<strong>APEX AI:</strong> Connection timed out or key not configured. Please verify your internet connection.`;
...
function renderBotReply(text) {
    // Basic Markdown conversion for premium rendering of Gemini output
    let html = text;
    
    // Escape HTML first (except we trust our parser markup replacements)
    ...
    html = html.replace(/^\s*\*\s+(.*?)$/gm, "<li style='margin-left: 15px; margin-top: 4px; list-style-type: square;'>$1</li>");
    html = html.replace(/^\s*-\s+(.*?)$/gm, "<li style='margin-left: 15px; margin-top: 4px; list-style-type: square;'>$1</li>");

    // Convert line breaks to <br>
    html = html.replace(/\n/g, "<br>");

    // Fix lists inside br tags
