...
import os
from dotenv import load_dotenv  # <-- 1. Import dotenv first

# Initialize dotenv at the absolute top so all variables are ready in memory
load_dotenv()  # <-- 2. Run it immediately!

# Now it is perfectly safe to import your custom database and auth files
import database  # <-- 3. Safe to import now!
import auth
import jwt

# Load variables inside the hidden .env file into our system's memory environments
load_dotenv()
...
# 1. INITIALIZE APP AND RATE LIMITER
app = FastAPI(title="Calisthenics & Gym Tracker API")

# Setup the IP-based rate limiter
...
def get_current_user(access_token: str = Cookie(None)):
    ...
    try:
        ...
        # Pull the Secret Key dynamically out of environment configuration
...
def get_analytics(db: Session = Depends(database.get_db), current_user_id: int = Depends(get_current_user)):
    query = text("""
        SELECT CAST(workout_date AS DATE) as log_date, 
...
def google_auth(
    ...
    try:
        # Dynamic lookups for the public client profile container
...
def get_user_logs(
    ...
    if type:
        ...
        result = db.execute(text(query), {"uid": current_user_id, "wtype": type}).fetchall()
    else:
        query += " ORDER BY wl.workout_date DESC"
        result = db.execute(text(query), {"uid": current_user_id}).fetchall()
